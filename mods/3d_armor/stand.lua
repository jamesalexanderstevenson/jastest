...
local function update_entity(pos)
	...
	if object then
		if not string.find(node.name, "3d_armor:stand") then
			object:remove()
			return
		end
	else
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	...
	if fields.store then
		store_armor(player, forms.players[player:get_player_name()], pos)
	elseif fields.apply then
		apply_armor(player, forms.players[player:get_player_name()], pos)
	end
end)
...
minetest.register_abm({
	...
	nodenames = {"3d_armor:stand", "3d_armor:stand_locked"},
	interval = 15,
	chance = 1,
...
minetest.register_craft({
	...
	recipe = {
		{"3d_armor:stand", "default:steel_ingot"},
	}
})
