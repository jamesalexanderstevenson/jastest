...
function boat.on_step(self, dtime)
		...
		if driver_objref then
			...
			if ctrl.left then
				if self.v < -0.001 then
					self.object:set_yaw(self.object:get_yaw() - 0.06)--dtime * 0.9)
				else
					self.object:set_yaw(self.object:get_yaw() + 0.06)--dtime * 0.9)
				end
	...
	self.object:add_velocity({x = -velo.x, y = -velo.y, z = -velo.z})
...
minetest.register_craftitem("boats:boat_mese", {
	description = S("Mese Boat"),
	inventory_image = "boats_mese_inventory.png",
	--inventory_image = "boats_inventory.png^[colorize:yellow:95",
	wield_image = "boats_mese_wield.png",
	wield_scale = {x = 2, y = 2, z = 1},
	liquids_pointable = true,
	groups = {flammable = 2},
	...
	on_place = function(itemstack, placer, pointed_thing)
		...
		pointed_thing.under.y = pointed_thing.under.y + 0.5
		boat = minetest.add_entity(pointed_thing.under, "boats:boat_mese")
		if boat then
			if placer then
				boat:set_yaw(placer:get_look_horizontal())
			end
