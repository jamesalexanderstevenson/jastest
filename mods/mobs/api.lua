...
function mob_class:set_velocity(v, up)
...
function mob_class:item_drop()
	...
	if death_by_player then
		local p = self.cause_of_death.puncher
		local d = self.damage
		local y = p:get_pos().y
		if y <= -1000 then
			d = d * 2
		...
		minetest.after(0.1, classes.addxp, p, d)
	end
...
function mob_class:check_for_death(cmi_cause)
	...
	if self.health > 0 then
		...
		if show_health and (cmi_cause and cmi_cause.type == "punch") then
			self.htimer = 2
			if not self.name:find("monster") then
...
function mob_class:is_at_cliff()
	...
	local tt, ttt = minetest.line_of_sight(
		...
		{x = pos.x + dir_x, y = ypos - self.fear_height, z = pos.z + dir_z}, 1)
	if tt then
		return true
	end
	...
	if nn and nn.name then
		...
		if nndef and nndef.drawtype and
			...
			--self:set_yaw(yaw + random(-0.75, 0.75), 1)
			self:set_yaw(-yaw + random(-0.15, 0.15), 1)
			self.state = "walk"
			return true
		end
	end
...
function mob_class:replace(pos)
	...
	if not mobs_griefing or not self.replace_rate or
			...
			not self.object:get_velocity() or
...
function mob_class:follow_flop()
	...
	if self.fly then
		...
		if not self:attempt_flight_correction() then
			...
			self.state = "flop"
			self.object:add_velocity({x = 0, y = -1, z = 0})
				...
				if not self.custom_attack then
					...
					if self.timer > 1 then
						...
						if self:line_of_sight(p2, s2) == true then
							...
							hud.message(self.attack, "+" .. d .. " DMG")
							self.attack:punch(self.object, 1.0, {
									full_punch_interval = 1.0,
									damage_groups = {fleshy = d}}, nil)
						else
							self:set_velocity(5, 5)
							self.object:add_velocity({
								x = self.object:get_yaw(),
								y = 1,
								z = self.object:get_yaw()
							})
						end
					end
...
function mob_class:on_punch(hitter, tflp, tool_capabilities, dir)
	...
	and minetest.is_protected(self.object:get_pos(), hitter:get_player_name()) then
		hud.message(hitter, S("Mob has been protected!"))
...
function mob_class:get_staticdata()
	if self.master then
		if minetest.get_player_by_name(self.master) and mobs.players[self.master] then
			mobs.players[self.master].object = self.object
			mobs.players[self.master].ent = self.object:get_luaentity()
		end
	end
	...
	and self.lifetimer < 20000 then
		...
		local nep = vector.new(self.object:get_pos())
		if nep then
			minetest.after(0.5, mobs.loho, nil, nep)
		end
		self.object:remove()
...
function mobs:capture_mob(self, clicker, chance_hand, chance_net, chance_lasso,
	...
	and force_take == false then
		...
		hud.message(clicker, S("@1 is owner!", self.owner))
	...
	if clicker:get_inventory():room_for_item("main", mobname) then
		...
		elseif chance then
			hud.message(clicker, S("Missed! Try again"))
...
function mobs:protect(self, clicker)
	...
	if self.protected == true then
		...
		hud.message(clicker, S("Already protected!"))
		return true -- false
	end
