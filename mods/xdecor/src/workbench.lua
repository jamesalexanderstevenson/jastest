...
function workbench.dig(pos, player)
	local inv = minetest.get_meta(pos):get_inventory()
	return inv:is_empty("input") and inv:is_empty("hammer") and
			inv:is_empty("tool") and inv:is_empty("storage") and
...
xdecor.register("workbench", {
	...
	on_skeleton_key_use = function(pos, player)
		...
		if p == 0 then
			...
			meta:set_string("infotext", "Work Bench (Locked by " ..
					owner .. ")")
			meta:set_int("locked", 1)
			hud.message(player, "Locked Work Bench")
		else
			meta:set_string("infotext", "Work Bench")
			meta:set_int("locked", 0)
			hud.message(player, "Unlocked Workbench")
		end
	end,
})
