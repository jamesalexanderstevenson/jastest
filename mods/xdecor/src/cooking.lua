...
function cauldron.filling(pos, node, clicker, itemstack)
	...
	if wield_item:sub(1,7) == "bucket:" then
		...
		elseif wield_item:sub(-6) == "_water" and node.name:sub(-6) == "_empty" then
			...
			itemstack:replace(wield_item:sub(1, -6) .. "empty")
		end
...
function cauldron.take_soup(pos, node, clicker, itemstack)
	...
	if item_name == "crops:clay_bowl" then
		if wield_item:get_count() > 1 then
			if inv:room_for_item("main", "crops:vegetable_stew 1") then
				itemstack:take_item()
				inv:add_item("main", "crops:vegetable_stew 1")
			else
				hud.message(clicker,
			...
			itemstack:replace("crops:vegetable_stew 1")
		end
...
-- Craft items
minetest.register_alias("xdecor:bowl", "crops:clay_bowl")
--[[
...
minetest.register_craftitem("xdecor:bowl_soup", {
	...
	stack_max = 1,
	on_use = minetest.item_eat(67, "xdecor:bowl")
})
