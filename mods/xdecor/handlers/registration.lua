	...
	if inventory then
		def.on_construct = def.on_construct or function(pos)
			...
			meta:set_string("formspec",
					(inventory.formspec or
					get_formspec_by_size(size)) .. xbg)
		end
		def.after_place_node = def.after_place_node or
				function(pos, placer, itemstack, pointed_thing)
			local meta = minetest.get_meta(pos)
			meta:set_string("owner", placer:get_player_name())
		end
		def.on_skeleton_key_use = def.on_skeleton_key_use or
				function(pos, player)
			local meta = minetest.get_meta(pos)
			local owner = meta:get_string("owner")
			local p = meta:get_int("locked")
			if player:get_player_name() ~= owner then
				return
			end
			if p == 0 then
				local it = meta:get_string("infotext")
				meta:set_string("infotext", it .. " (Locked by " ..
						owner .. ")")
				meta:set_int("locked", 1)
				hud.message(player, "Locked " .. def.description)
			else
				meta:set_string("infotext", def.infotext or "")
				meta:set_int("locked", 0)
				hud.message(player, "Unlocked " .. def.description)
			end
		end
