...
multiskin.set_player_skin = function(player, skin)
	...
	if format then
		...
		player:get_meta():set_string("multiskin_format", format)
	end
	if not player_skins[name] or not player_skins[name].skin then
		return
	end
...
multiskin.get_preview = function(player)
	local skin = player:get_meta():get("multiskin_skin")
	if skin then
...
multiskin.update_player_visuals = function(player)
	...
	local skin = player_skins[name].skin or "3d_armor_trans.png"
	...
	if format == "1.8" then
		...
		textures[1] = cape or "3d_armor_trans.png"
		textures[2] = skin
	else
		if cape then
			skin = skin.."^"..cape
...
minetest.register_chatcommand("multiskin", {
	...
	func = function(name, param)
		...
		elseif cmd == "unset" and player then
			...
			player:get_meta():set_string("multiskin_skin", nil)
			multiskin.update_player_visuals(player)
		else
