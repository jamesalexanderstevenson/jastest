...
minetest.register_node("fire:antifire", {
	description = "Antifire",
	drawtype = "firelike",
	tiles = {{
...
minetest.register_abm({
	...
	action = function(pos, node, active_object_count, active_object_count_wider)
		local p1 = {x = pos.x - 3, y = pos.y - 3, z = pos.z - 3}
		local p2 = {x = pos.x + 3, y = pos.y + 3, z = pos.z + 3}
		local n = minetest.find_nodes_in_area(p1, p2, {"fire:basic_flame"})
		for i = 1, #n do
			minetest.set_node(n[i], {name = "fire:antifire"})
		end
		minetest.after(math.random() + math.random() + math.random(), function()
...
flame_fire_node.on_timer = function(pos)
	...
	if minetest.is_protected(pos, "") then
		if math.random() >= 0.25 then
			...
			minetest.swap_node(pos, {name = "fire:antifire"})
		end
	end
	-- Restart timer
	return true
	...
	on_use = function(itemstack, user, pointed_thing)
		...
		if pointed_thing.type == "node" then
			...
			if minetest.is_protected(pointed_thing.under, player_name) then
				...
				hud.message(user, "This area is protected")
				return
			end
