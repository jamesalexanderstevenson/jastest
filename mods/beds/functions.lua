...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	if formname == "beds:form" then
		...
		if fields.sleep then
			local pos = beds.sel[name]
			if pos then
				beds.on_rightclick(pos, player)
				beds.sel[name] = nil
			end
		elseif fields.setrespawn then
			beds.player[name] = player:get_pos()
			beds.set_spawns()
			beds.player[name] = nil
			forms.dialog(name, "Respawn position has been set!", true)
		elseif fields.sethome then
			local s = sethome.set(name, player:get_pos())
			if s then
				forms.dialog(name, "Home position has been set!", true)
			end
		end
	end
